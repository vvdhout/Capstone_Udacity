...
contract('Verifier', async accounts => {
  ...
  describe('test verifier', function () {
      ...
      it('verifies correct proof', async function () {
          this.contract = await Verifier.deployed({from: account_one});
          let boolean = await this.contract.verifyTx.call(
          ...
          assert.equal(boolean, true, "Does not verify correct proof.");
      })

      it('does not verify with incorrect proof', async function () {
          this.contract = await Verifier.deployed({from: account_one});
          let boolean = await this.contract.verifyTx.call(
            ["0x15e40e803f92f0e7f1f28b8a73610de34ddc99988cd76b3aa8b248267516a71e", "0x03d3e6d9fa2f4a4b88d42eeb45e079e2adc2f7bfc6e20a8abc5dc76baf436445"],
        		["0x1b5365e0b797c4ee7dbeff238f1f617662376fb07cff0f8da7d0c76f03321dac", "0x04fd329a6cdec5855190e744ff03e3546d32548c6f2838c3cd7369531694809f"],
          ...
          assert.equal(boolean, false, "Verifies even when incorrect proof.");
      })
